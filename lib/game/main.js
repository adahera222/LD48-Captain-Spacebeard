...
.requires(
	...
	'game.levels.menu',
	'game.levels.asteroidone',
	'game.levels.asteroidtwo'
...
MyGame = ig.Game.extend({
	...
	init: function() {
		...
		this.loadLevel( LevelAsteroidone );
	},
	...
	loadLevel: function( data ) {
		...
		this.spawnEntity(shipChoice, 225, 1920, {player:true});
...
MyTitle = ig.Game.extend({
	...
	init: function() {
		...
		this.spawnEntity( EntityCloakship, ig.system.width/2 + 96 - 9 + 42, ig.system.height/2 + 60, {player:false,angle:1.57079632679489661923132169163975144});
	},
	...
	draw: function() {
		...
		this.title.draw( cx - this.title.width/2, ig.system.height/2 - this.title.height/2 - 40 );
		this.font.draw('DELIVER TO THIS GUY:', ig.system.width/2+96+42, ig.system.height/2+40, ig.Font.ALIGN.CENTER);
		this.font.draw('CHOOSE YOUR SHIP:', ig.system.width/2-32, ig.system.height/2+40, ig.Font.ALIGN.CENTER);
	}
});

MyEnding = ig.Game.extend({
	clearColor: "#131214",
	gravity: 0,
	...
	init: function() {
		...
		this.spawnEntity( EntityCloakship, ig.system.width/2 - 40, ig.system.height/2-60, {player:false,angle:1.57079632679489661923132169163975144});
		this.spawnEntity(shipChoice, ig.system.width/2 + 40, ig.system.height/2 - 60, {player:true})
	},
	...
	draw: function() {
		...
		this.font.draw('"YOU\'VE GOT THE CARGO, I TAKE IT?\nARR, THANKS MATEY!"', ig.system.width/2-40, ig.system.height/2 - 20-60, ig.Font.ALIGN.CENTER);
		this.font.draw('<LONG RANGE TRANSMISSION FROM THE SPACE POLICE>\n\n"WE SEEM TO HAVE AN ALIEN PROBLEM TO ATTEND TO,\nBUT WE\'LL CATCH YOU NEXT TIME SPACEBEARD!"', ig.system.width/2, ig.system.height/2 + 30, ig.Font.ALIGN.CENTER);
		
		this.font.draw('A GAME FOR LUDUM DARE #28 BY DALE PRICE (TWITTER: @DAPRICE)', ig.system.width/2, ig.system.height - 20, ig.Font.ALIGN.CENTER);
	}
});
...
// If our screen is smaller than 1000px in width (that's CSS pixels), we scale the 
...
// with small screens.
var scale = (window.innerWidth < 1000) ? 1 : 0.5;
